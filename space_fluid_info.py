...
class INFO_MT_scenes(Menu):
    bl_label = "Scenes"    
    
    def draw(self, context):
        layout = self.layout
        for scene in bpy.data.scenes:
            layout.operator("fd_scene.change_active_scene",text=scene.name,icon='SCENE_DATA').scene_name = scene.name
...
class OPS_load_blender_defaults(bpy.types.Operator):
...
class OPS_save_startup_file(bpy.types.Operator):
